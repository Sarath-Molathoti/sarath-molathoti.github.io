document.addEventListener("DOMContentLoaded", (event) => {
  document.getElementById("compareButton").addEventListener("click", () => {
    ...
    originalStats.innerHTML = diff.originalStats;
    modifiedStats.innerHTML = diff.modifiedStats;
  });
  ...
  document.getElementById("resetButton").addEventListener("click", () => {
    ...
    document.getElementById("modifiedStats").innerHTML = "";
  });
});
...
function getDiff(text1, text2) {
  ...
  for (let i = 0; i < maxLength; i++) {
    ...
    removedCount += (lineDiff.original.match(/<span class="removed">/g) || [])
      .length;
    addedCount += (lineDiff.modified.match(/<span class="added">/g) || [])
      .length;

    diff.original.push(`<div>${lineDiff.original}</div>`);
    diff.modified.push(`<div>${lineDiff.modified}</div>`);
  }

  const originalStats = getTextStats(text1);
  const modifiedStats = getTextStats(text2);

  diff.originalStats.unshift(`
        <div class="stats">
  ...
  return {
    ...
    originalStats: diff.originalStats.join("\n"),
    modifiedStats: diff.modifiedStats.join("\n"),
  };
}
...
function diffLines(line1, line2) {
  ...
  diff.forEach((part) => {
    ...
    if (part.removed) {
      originalLine += `<span class="${className}">${escapeHtml(
        part.value
      )}</span>`;
    } else if (part.added) {
      modifiedLine += `<span class="${className}">${escapeHtml(
        part.value
      )}</span>`;
    } else {
